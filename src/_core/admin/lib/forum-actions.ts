...
import {forumBoard, forumCategory} from "@/_feature/forum/schema/forum.schema";
...
/**
 * Formats comma-separated text strings down to structured array maps containing cleaned key sequences. Strips out
 * blank entries or excess surrounding space characters safely.
 *
...
export async function reorderForumBoardsAction(formData: FormData): Promise<void> {
    ...
    // pull verified records currently residing within the category context to act as standard boundaries
...
export async function removeBoardFromCategoryAction(formData: FormData): Promise<void> {
    ...
    // auto-creates a baseline isolation structure if missing from data layers
    if (!uncategorizedId) {
